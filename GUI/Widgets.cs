...
namespace DirectDimensional.Editor.GUI {
    public static unsafe class Widgets {
        ...
        public static void TextWrapped2(Rect rect, ReadOnlySpan<char> text, HorizontalTextAnchor anchorH = HorizontalTextAnchor.Left, VerticalTextAnchor anchorV = VerticalTextAnchor.Top) {
            if (ImGui.CurrentWindow == null || text.IsEmpty || rect.HasInvalidSize || !Utils.LocalIntersectScissorRect(rect)) return;
        ...
        public static void TextWrapped(Rect rect, ReadOnlySpan<char> text, HorizontalTextAnchor anchorH = HorizontalTextAnchor.Left, VerticalTextAnchor anchorV = VerticalTextAnchor.Top) {
            switch (anchorH) {
                default:
                    TextWrappedLeftAnchor(rect, text, anchorV);
                    break;

                case HorizontalTextAnchor.Middle:
                    TextWrappedMiddleAnchor(rect, text, anchorV);
                    break;

                case HorizontalTextAnchor.Right:
                    TextWrappedRightAnchor(rect, text, anchorV);
                    break;
            }
        }
        private static void TextWrappedLeftAnchor(Rect rect, ReadOnlySpan<char> text, VerticalTextAnchor anchorV) {
            if (ImGui.CurrentWindow == null || text.IsEmpty || rect.HasInvalidSize || !Utils.LocalIntersectScissorRect(rect)) return;
            ...
            fixed (stbtt_packedchar* pPacked = &EditorResources.PackedChar[0]) {
                ...
                switch (anchorV) {
                    default: {
                        float xPosition = rect.Position.X;
                        float yPosition = rect.Position.Y + (ascent + lineGap) * scale;
                        float oldX = xPosition;

                        bool exceedMaxY = false;
                        ...
                        while (begin < text.Length) {
                            ...
                            if (begin != 0) {
                                while (char.IsWhiteSpace(text[begin])) {
                                    begin++;
                                }
                            }

                            var wrap = begin + Utilities.CalcWordWrapIndex(text[begin..], rect.Width);
                            var slice = text[begin..wrap];
                            ...
                            for (int i = 0; i < slice.Length; i++) {
                                ...
                                switch (c) {
                                    case ' ':
                                        xPosition += spaceAdvance;
                                        continue;
                                    case '\t':
                                        xPosition += spaceAdvance * 4;
                                        continue;
                                    case '\n':
                                        xPosition = oldX;

                                        if (yPosition >= rectMax.Y)
                                            exceedMaxY = true;
                                        else
                                            yPosition += lineSpace;
                                ...
                                ImGuiRender.AddCharacterQuad_Raw(xPosition, yPosition, ptr, uvStep, textCol);
                                xPosition += ptr->xadvance;
                            }

                            xPosition = oldX;
                            yPosition += lineSpace;

                            if (yPosition >= rectMax.Y) break;
                    ...
                    case VerticalTextAnchor.Middle: {
                        ...
                        bool subceedMinY = yPosition < rect.Position.Y;
                        bool exceedMaxY = false;
                                ...
                                switch (c) {
                                    ...
                                    case '\n':
                                        ...
                                        if (yPosition >= rectMax.Y)
                                            ...
                                            yPosition += lineSpace;
                                            subceedMinY = yPosition < rect.Position.Y;
                                        }
        ...
        private static void TextWrappedRightAnchor(Rect rect, ReadOnlySpan<char> text, VerticalTextAnchor anchorV) {
            if (ImGui.CurrentWindow == null || text.IsEmpty || rect.HasInvalidSize || !Utils.LocalIntersectScissorRect(rect)) return;
                ...
                switch (anchorV) {
                    default: {
                        ...
                        while (begin < text.Length) {
                            ...
                            {
                                var buildCurrent = 0;
                                while (buildCurrent < slice.Length) {
                                    int linebreak = slice[buildCurrent..].IndexOf('\n');
                                    linebreak = linebreak == -1 ? slice.Length - buildCurrent : linebreak;

                                    var segment = slice.Slice(buildCurrent, linebreak);
                                    var segmentWidth = Utilities.CalcStringWidth(segment);

                                    float xPosition = rectMax.X - segmentWidth;
                                    ...
                                    for (int i = 0; i < segment.Length; i++) {
                                        ...
                                        switch (c) {
                                            ...
                                            case '\t':
                                                ...
                                                continue;
                                            case '\r':
                                                xPosition = rectMax.X - segmentWidth;
                                                continue;
                                        }
                                    ...
                                    //exceedMaxY = yPosition >= rectMax.Y;
                                    yPosition += lineSpace;

                                    buildCurrent += linebreak + 1;
                                }
                            }
                            ...
                            //    ImGuiRender.AddCharacterQuad_Unchecked(xPosition, yPosition, ptr, uvStep, textCol);
                            //    xPosition += ptr->xadvance;
                            //}
        ...
        private static void TextWrappedMiddleAnchor(Rect rect, ReadOnlySpan<char> text, VerticalTextAnchor anchorV) {
